...
export class KanbanChecker{
    constructor(private localPath: string) { checkIsDir(this.localPath); }

    public kanbanCheckers() {
        checkIsFile(path.join(this.localPath, 'task.ts'));
        checkFileContent(path.join(this.localPath, 'task.ts'), `import { defineStore } from 'pinia';
// project imports
